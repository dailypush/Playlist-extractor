...
class CacheTests(unittest.TestCase):
    def test_persistence_provider_isolation_and_negative_expiry(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIsNone(cache.get('another-provider', 'a'))
    ...
    def test_duplicate_recording_makes_no_new_requests(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            one.touch()
            two.touch()
            with patch.object(scanner, 'duration', return_value=60), patch.object(scanner, 'sample', side_effect=fake_sample), patch.object(scanner.shutil, 'which', return_value='/tool'), patch.object(scanner, 'recognize', return_value=[track()]) as api:
